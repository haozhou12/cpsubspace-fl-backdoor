...
seeds with no trust advantage ($1.04\times$ the honest score). Aiming the cover at the estimated anchor makes the attack reliable: our construction succeeds in all five tested seeds ($0.982\pm0.014$) by earning a $1.44\times$ trust advantage, against $3/5$ for the naive attempt. A random-direction cover, whose trust falls far below the honest score, succeeds in $0/5$ at $\tau=0.97$. The construction does not break an otherwise-robust defense; it turns an unreliable leak into a consistent one, and we identify the mechanism --- the anchor-aligned cover --- that does it. We release
the harness and all raw results at
\url{https://github.com/haozhou12/cpsubspace-fl-backdoor}.
\end{abstract}
...
with the reference --- but unreliably. A closed-form cover aimed at the estimated reference lifts the adversary's trust above the honest score and plants the backdoor consistently across seeds. \emph{Related work.} FL backdoors have been built by model
replacement~\cite{bagdasaryan}, an adversarial lens~\cite{bhagoji}, and
distributed triggers~\cite{dba,badnets}, all optimizing the poisoned update; ours
is closed form. Robust aggregation --- Krum~\cite{krum}, trimmed
mean~\cite{trimmedmean}, similarity-based trust~\cite{fltrust,rflpa} --- filters
malicious updates but remains vulnerable to shaped local poisoning~\cite{poisoning},
a robustness question with a signal-processing
lineage~\cite{spl_fl1}. High-dimensional freedom has been
used against Byzantine aggregators~\cite{hidra} and within benign
variance~\cite{alie}; we target the private-anchor trust score itself. Our contributions are:
...
\label{sec:model}
\noindent\textbf{Aggregation.} $n$ clients run synchronous FL~\cite{fedavg}; in round $t$ client $i$ submits $g_i\in\mathbb{R}^d$ under SecAgg~\cite{secagg}. The server computes a reference
...
GroupNorm~\cite{groupnorm} (BatchNorm statistics are buffers, not aggregated under update-based FL), $20$ clients with an IID split disjoint from a
...
$0.05$ of its final value by round~$25$ (mean ASR $0.20$ there), while seeds first
reach ASR $0.5$ at rounds $25$--$44$; a $40$-round budget yields $0.512\pm0.267$.
We do not claim the onset is predictable.
...
search. Its clean-accuracy cost --- $2.6$ points at $\tau=0.97$ on Fashion-MNIST, smaller at $\tau=0.7$, larger on CIFAR-10 --- is not the price of the backdoor payload: a random or absent payload costs nearly as much while planting nothing, though we do not fully separate the cover and payload contributions. The analysis suggests two failure mechanisms, clamping of a poor anchor estimate and payload starvation, and both appear in the sweep. A defense that scores only a scalar projection cannot see the payload;
...
enable misuse: the construction needs control of a large client fraction and degrades clean accuracy, and is plausibly countered by a defense that also scores the component orthogonal to the reference --- a direction we leave to future work. We follow responsible-disclosure practice and release our code to support verification and the development of countermeasures.
