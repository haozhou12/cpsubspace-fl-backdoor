...
def run(rule, seed, tau=0.97, M=20, mf=0.3, rounds=40, root_n=100,
    ...
    for r in range(rounds):
        ...
        if attack == "cpsub" and nmal > 0:
            ...
            v = v / (v.norm() + 1e-12)
            if os.environ.get("PAYLOAD") == "random":
                # INTERVENTION: same tau, same cover, payload replaced by a random
                # direction orthogonal to e0h. Tests whether the payload direction
                # (not merely its budget) affects clean accuracy.
                v = torch.randn_like(v); v = v - torch.dot(v, e0h) * e0h; v = v / (v.norm() + 1e-12)
