...
def run(rule, seed, tau=0.97, M=20, mf=0.3, rounds=40, root_n=100,
    ...
    for r in range(rounds):
        ...
        gmal = None
        if attack == "naive" and nmal > 0:
            # BASELINE (a): submit the poisoned local update directly -- no cover,
            # no orthogonal projection, no norm-matching. Colluders share it (same
            # data/objective as cpsub); the ONLY difference from cpsub is the
            # cover+projection construction. The defense normalizes it to ||g0||.
            gmal = local(w, mpool, steps=steps, poison=True) - w
            cos_anchor = float("nan")
        elif attack == "cpsub" and nmal > 0:
            gp = local(w, mpool, steps=10) - w           # gray-box anchor estimate
            e0h = gp / (gp.norm() + 1e-12)
            if os.environ.get("COVER") == "rand":
                # CONTROL (c): replace the estimated anchor with a random unit
                # direction. Tests whether aiming the cover at the reference matters,
                # or any norm-matched positive-cosine direction earns the same trust.
                rvec = torch.randn_like(e0h); e0h = rvec / (rvec.norm() + 1e-12)
            b = local(w, mpool, steps=steps, poison=True) - w
            v = b - torch.dot(b, e0h) * e0h
            v = v / (v.norm() + 1e-12)
            gmal = (tau * e0h + math.sqrt(1 - tau * tau) * v) * n0
            cos_anchor = float(torch.dot(e0h, g0) / n0)   # cover-to-g0 cosine (est or random)
        else:
            cos_anchor = float("nan")

        ups = []
        for i in range(M):
            if i in mal and attack in ("cpsub", "naive"):
